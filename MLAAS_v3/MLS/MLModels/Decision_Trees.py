        ...
        try:
            if request.POST['submit'] != "RandomSearch":
                ...
                if request.POST['submit'] == "TRAIN" or request.POST['submit'] == "TRAIN_Rand":
                    ...
                    container.start()
                    log = container.exec_run("python3 Major/Docker/Decision_Trees.py {} 1".format(user),
                    ...
                    container = client.containers.create("mlaas",
                                                         ...
                                                         tty=True, stdin_open=True, auto_remove=False)
                    container.start()
                    log = container.exec_run("python3 Major/Docker/Decision_Trees.py {} 2".format(user),
                                             stderr=True, stdout=True, stream=True)
                    container.stop()
                    container.remove()
                    with open('media/user_{}/Variable/result.pkl'.format(user), 'rb') as f:
                ...
                container = client.containers.create("mlaas",
                                                     ...
                                                     tty=True, stdin_open=True, auto_remove=False)
                container.start()
                log = container.exec_run("python3 Major/Docker/Decision_Trees.py {} 3".format(user),
                                         stderr=True, stdout=True, stream=True)
                container.stop()
                container.remove()
                with open('media/user_{}/Variable/result.pkl'.format(user), 'rb') as f:
