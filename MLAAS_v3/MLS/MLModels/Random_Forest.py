        ...
        try:
            if request.POST['submit'] != "RandomSearch":
                ...
                if request.POST['submit'] == "TRAIN" or request.POST['submit'] == "TRAIN_Rand":
                    ...
                    container.start()
                    log = container.exec_run("python3 Major/Docker/Random_Forest.py {} 1".format(user),
                    ...
                    container = client.containers.create("mlaas",
                                                         ...
                                                         tty=True, stdin_open=True, auto_remove=False)
                    container.start()
                    log = container.exec_run("python3 Major/Docker/Random_Forest.py {} 2".format(user),
                                             stderr=True, stdout=True, stream=True)
                    container.stop()
                    container.remove()
                    with open('media/user_{}/Variable/result.pkl'.format(user), 'rb') as f:
                ...
                container = client.containers.create("mlaas",
                                                     ...
                                                     tty=True, stdin_open=True, auto_remove=False)
                container.start()
                log = container.exec_run("python3 Major/Docker/Random_Forest.py {} 3".format(user),
                                         stderr=True, stdout=True, stream=True)
                for line in log[1]:
                    line = line.decode()
                ...
                return render(request, 'MLS/RandomSearch.html', {"model": "Random_Forest", "parameters": parameters,
                                                                 "features": features_list, "label": label,
                                                                 "filename": file_name})
